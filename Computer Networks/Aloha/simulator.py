...
class Simulator:
    ...
    class Channel(api.Channel):
        ...
        def carrier_sense(self):
            return self.simulator.is_someone_sending
        ...
        def tick(self, tick):
            if self.frame is None:
                if random.random() < self.frame_probability:
                    ...
                    self.simulator.number_of_frames += 1
                else:
            ...
            if len(transfers) == 1 and self.id in transfers and self.channel._transmit_part is not None:
                if self.channel._transmit_part == self.next_part:
                    ...
                    if self.frame is not None and self.next_part == self.frame.length:
                        ...
                        self.frame = None
                        self.next_part = 0
                        self.simulator.number_of_success += 1
                    return
    ...
    def tick(self):
        ...
        self.is_someone_sending = len(transfers) > 0
    ...
    def stats(self):
        ...
        result['number_of_frames'] = self.number_of_frames
        result['number_of_success'] = self.number_of_success 
        result['effectiveness'] = self.number_of_success / self.number_of_frames * 100
        return result