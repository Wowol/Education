...
class Simulator:
    ...
    class SimRouter(api.Router):
        def __init__(self, algorithm_class, id=None):
            if not issubclass(algorithm_class, api.Algorithm):
                raise ValueError
            super().__init__()
        ...
        def drop_packet(self, packet):
            ...
            logging.info(
                "Droped packet [{}] {} -> {}".format(packet.id, packet.src, packet.dst))
    ...
    def stats(self):
        ...
        if self.routable_packets > 0:
            response['delivery_rate'] = len(
                self.routed_packets) / self.routable_packets
        response['routed'] = len(self.routed_packets)
        if len(self.routed_packets) > 0:
            response['avg_time'] = sum(
                [p.stop_time - p.start_time for p in self.routed_packets]) / len(self.routed_packets)
        return response
        ...
        for id, router in self.routers.items():
            ...
            for src, packet in router.packets.values():
                if packet.dst != router.id:
                    logging.warning("Silently droped packet [{}] {} -> {} at {}".format(
                        packet.id, packet.src, packet.dst, router.id))
            router.packets = dict()
        for id, router in self.routers.items():
                ...
                if link.packet is not None:
                    ...
                    if link.dst in self.routers:
                        if isinstance(packet, Simulator.SimPacket) and packet.dst == link.dst:
                            ...
                            logging.info("Routed packet [{}] {} -> {} in {} steps".format(
                                packet.id, packet.src, packet.dst, packet.stop_time - packet.start_time))
                        else:
                            logging.debug(
                                "Forwarded packet [{}] {} -> {} to {}".format(packet.id, packet.src, packet.dst, link.dst))
                            self.routers[link.dst].packets[packet.id] = (
                                self.routers[link.dst]._links[router.id], packet)
