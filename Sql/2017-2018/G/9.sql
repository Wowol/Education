...
FROM pracownicy p CROSS JOIN etaty e
WHERE (id, pensja_do) IN (
    select id, MAX(pensja_do) FROM pracownicy p CROSS JOIN etaty e
    WHERE pensja_od <= pensja+COALESCE(dodatki, 0) AND pensja_do >= pensja+COALESCE(dodatki, 0)
    GROUP BY id
) AND pensja_od <= pensja+COALESCE(dodatki, 0)
